...
{
    public class AugurEntityWithGridControllerTsFt
    {
        private readonly Model model;
        private readonly IList<string> baseEndpointNames;

        public AugurEntityWithGridControllerTsFt(Model model, IList<string> baseEndpointNames)
        {
            this.model = model;
        ...
        private string GetEndpointTemplates(EndpointModel endpoint)
        {
            ...
            if (returnType != null && returnType.IsGenericType && typeof(GridResult<>).MakeGenericType(returnType.GenericTypeArguments).IsAssignableFrom(returnType))
            {
                //NOTE: This doesn't work with custom HTTP headers (hasCustomHeaders) so beware!
                //Should add support for this if the need arises.
                returnTypeStr = $"AugurHttpRequest<GridResult<{GetReturnTypeStr(returnType.GenericTypeArguments.First().ToDescriptionModel().TypescriptType, hasCustomHeaders)}>>";
                genericReturnModel = $"<GridResult<{GetReturnTypeStr(returnType.GenericTypeArguments.First().ToDescriptionModel().TypescriptType, hasCustomHeaders)}>>";
            }
        ...
        public string GenerateText()
        {
                        ...
                        (
                            this.model.Endpoints
                                ...
                                .SelectMany(t =>
                                {
                                    if (t.IsGenericType)
                                    {
                                        return t.GetGenericArguments().Select(x => x.ToDescriptionModel());
                                    }

                                    return new List<TypeDescription> { t.ToDescriptionModel() };
                                })
                        )
                        .Append(this.model.Entity)
                        .Append(this.model.GridModel)
                        .Where(m => m.IsModelType)
                        .Where(m => typeof(GridRequestModel) != m.Type)
                        .Select(m => m.IsCollection ? m.GenericArgument : m)
                        .Where(p => !p.Type.IsGenericType || !typeof(GridResult<>).MakeGenericType(p.Type.GenericTypeArguments).IsAssignableFrom(p.Type))
                        .DistinctBy(m => m.Type)
                        .ToList();
            ...
            return $@"
                ...
                export class {this.model.Name} extends AugurEntityWithGridController<{this.model.Entity.TypescriptType}, {this.model.GridModel.TypescriptType}> {{
        ...
        public class Model
        {
            ...
            public TypeDescription GridModel { get; set; }
