...
{
    public class EntityTsFt
    {
        private readonly TypeDescription model;

        public EntityTsFt(TypeDescription model)
        {
            this.model = model;
        } 
            ...
            return $@"
                import {{ IAugurEntityWithId }} from '@augur';
                {
                    this.model.Properties
                        ...
                        .JoinBy("\r\n                ")
                }{ separator }export class {this.model.Type.Name.ToPascalCase()} implements IAugurEntityWithId {{
                    
                    constructor(init?: Partial<{this.model.Type.Name.ToPascalCase()}>) {{
                        {
                            this.model.Properties
                                .Where(p => p.Type.IsCollection)
                                .Select(p => $"this.{p.Name.ToCamelCase()} = [];")
                                .JoinBy("\r\n                        ")
                        }
